...
export const getAllCategories = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${URL}/api/v1/categories/getall`);
    dispatch({
...
export const adminGetCategoryById = (categoryId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.get(`${URL}/api/v1/categories/${categoryId}`);
...
export const adminDeleteCategoryById = (categoryId) => async (dispatch) => {
  try {
    ...
    await axios.delete(`${URL}/api/v1/categories/${categoryId}`);
    dispatch({
